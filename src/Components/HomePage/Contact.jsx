...
export default function Contact() {
  const [isCustomerHelp,setisCustomerHelp] = useState(false)
  const [isStudentHelp,setisStudentHelp] = useState(false)
  return (
    <Box sx={{p:2}}>
      {
        isStudentHelp && <StudentCare isStudentHelp={isStudentHelp} setisStudentHelp={setisStudentHelp}/>
      }
      {
        isCustomerHelp && <CustomerCare isCustomerHelp={isCustomerHelp} setisCustomerHelp={setisCustomerHelp}/>
      }
        <Paper elevation={2} sx={{p:2,mb:2}}>
            ...
            <Box sx={{display:'grid',gridTemplateColumns:'60% 40%',width:'100%',height:'40vh',boxSizing:'border-box',overflow:'hidden',bgcolor:'primary.dark',borderRadius:'8px'}}>
                ...
                <Typography variant='h6' sx={{color:'#fff',width:'70%',textAlign:'justify'}}>Whether you have questions, feedback, or just want to say hello, we're here and ready to assist</Typography>
                </Box>
            ...
            <Box sx={{display:'flex',boxSizing:'border-box'}}>
              ...
              <Box sx={{my:2,position:'relative',width:'100%'}}>
                ...
                <Box sx={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px, 250px))',justifyContent:'space-around',gap:2,mt:2}}>
                  ...
                  <Typography sx={{color:'primary.light'}}>If You are our Student and Having Any issue Retaled to Your Account We are Available 24/7</Typography>
                    <Button variant='contained' size={'small'} onClick={()=>setisCustomerHelp(true)}>Costumer Care</Button>
                  </Paper>
                  <Paper elevation={2} sx={{p:2,display:'flex',flexDirection:'column',justifyContent:'center',alignItems:'center',rowGap:1,maxWidth:'200px'}}>
                  ...
                  <Button variant='contained' size={'small'} onClick={()=>setisStudentHelp(true)}>Students Help</Button>
